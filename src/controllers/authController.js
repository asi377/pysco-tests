...
const HARDCODED_ADMIN_EMAIL = process.env.HARDCODED_ADMIN_EMAIL;
const HARDCODED_ADMIN_TOKEN = process.env.HARDCODED_ADMIN_TOKEN;
...
const login = catchAsync(async (req, res, next) => {
    ...
    if (user.isGuest) {
        return next(
            new AppError('این حساب یک حساب مهمان است. لطفاً ثبت‌نام کنید', 401),
        );
    }
...
const refresh = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
        return next(
            new AppError('احراز هویت ناموفق . لطفاً دوباره وارد شوید', 401),
        );
    }
    ...
    try {
        ...
        return next(
            new AppError(
                'رفرش توکن نامعتبر یا منقضی شده است. لطفاً دوباره وارد شوید',
                401,
            ),
        );
    }
    ...
    if (decoded.type !== 'refresh' || !decoded.id) {
        return next(new AppError('احراز هویت ناموفق', 401));
    }
    ...
    if (!storedToken) {
        return next(
            new AppError(
                'رفرش توکن قبلاً استفاده یا باطل شده است. لطفاً دوباره وارد شوید',
                401,
            ),
        );
    }
