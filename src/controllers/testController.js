        ...
        res.json({
            ...
            data: {
                session: {
                    ...
                    total: questions,
                    progress: Math.round(
                        (existingSession.answers.length / questions) * 100,
                    ),
                },
            },
        });
...
const submitOneAnswer = async (req, res) => {
  try {
    ...
    if (session.isCompleted) {
      res.status(400).json({ success: false, message: 'این جلسه قبلاً تکمیل شده است' });
      return;
    }
    ...
    if (answer.questionNumber !== firstUnanswered.questionNumber) {
      res.status(400).json({
        success: false,
        message: `شما باید ابتدا به سوال ${firstUnanswered.questionNumber} پاسخ دهید`,
      });
      return;
    }
    ...
    const remainingQuestions = allQuestions.filter(q => !answeredIds.has(q._id.toString()) && q.questionNumber !== answer.questionNumber);
    ...
    if (remainingQuestions.length === 0) {
      ...
      res.json({
        ...
        data: {
          completed: true,
          sessionId: session._id,
          redirect: `/result/${test.slug}/${session._id}`,
        },
      });
      return;
    }
    ...
    console.error('Submit one answer error:', error);
    res.status(500).json({ success: false, message: 'خطای سرور' });
  }
};
