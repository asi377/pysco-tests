...
const isTokenBlacklisted = async (token) => {
    ...
    return await TokenBlacklist.exists({ tokenHash: hash });
};
...
const protect = async (req, res, next) => {
    try {
        ...
        if (!token) {
            return next(
                new AppError(
                    'لطفاً وارد شوید یا به عنوان مهمان ادامه دهید',
                    401,
                ),
            );
        }
        ...
        if (isTokenBlacklisted(token)) {
            return next(
                new AppError('توکن منقضی شده است. لطفاً دوباره وارد شوید', 401),
            );
        }
        ...
        if (decoded.id === ADMIN_HARDCODED_ID && decoded.role === 'admin') {
            req.user = {
                ...
                role: 'admin',
                email: process.env.ADMIN_HARDCODED_EMAIL,
                fullName: 'مدیر سیستم',
            };
            req.tokenType = 'user';
            return next();
